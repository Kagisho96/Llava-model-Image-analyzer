...
with st.sidebar:   
    # creates selectbox to pick the model we would like to use
    selected_option = st.selectbox('Select an option:', ['mistral:instruct', 'Other Options'])

if selected_option == 'mistral:instruct':
    if chat_input := st.text_input("What would you like to ask?"):
        if not chat_input.strip():
            st.error('Please ask a question!')
            st.stop()

        # Processing the question, here you would implement your model
        # For this example, let's just echo the question
        st.write(f"You asked: {chat_input}")
